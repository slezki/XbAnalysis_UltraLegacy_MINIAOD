#include <Ponia/Onia/interface/OniaPhotonProducer.h>
...
void OniaPhotonProducer::produce(edm::Event& event, const edm::EventSetup& esetup){
  ...
  for (pat::CompositeCandidateCollection::const_iterator  dimuonCand = dimuons->begin(); dimuonCand!= dimuons->end(); ++dimuonCand){
     ...
     for (pat::CompositeCandidateCollection::const_iterator conv = conversions->begin(); conv!= conversions->end(); ++conv){
        ...
        float dMass = GetdM(chiCand,*dimuonCand);
        chiCand.addUserFloat("deltaM",dMass);

        candidates++;
        chiCand.addUserInt("index_Chi",candidates);
        chiCand.addUserInt("index_diMuInChi",diMuCount);
        chiCandColl->push_back(chiCand);  
     }
  }
  if (chiCandColl->size()>1) candidatesM++;
...
float OniaPhotonProducer::GetdM(const pat::CompositeCandidate& chiCand,  const pat::CompositeCandidate& dimuonCand){
  float deltamass = chiCand.p4().M() - dimuonCand.p4().M();
  return deltamass;
}
...
void OniaPhotonProducer::endJob(){
 ...
 // std::cout << "Delta mass fail: " << delta_mass_fail << std::endl;
  ...
  std::cout << "###########################" << std::endl;
  std::cout << "Found more than one Chi candidate in " << candidatesM << " events." << std::endl;
  std::cout << "###########################" << std::endl;
  std::cout << std::endl;
}
...
bool OniaPhotonProducer::cutDeltaMass(const pat::CompositeCandidate& chiCand,  const pat::CompositeCandidate& dimuonCand){
